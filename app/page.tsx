      ...
      <main className="max-w-4xl mx-auto p-4 md:p-10 pb-32 print:p-0">
          ...
          <div className="bg-white border border-slate-200 p-10 md:p-16 print:border-none print:p-0 min-h-[290mm] animate-in fade-in duration-700">
            <header className="flex justify-between items-end border-b-2 border-slate-950 pb-6 mb-12">
              ...
              <div className="text-right text-[9px] font-mono text-slate-400 uppercase tracking-widest font-bold leading-tight">
                {/* 以前の Täglicher Lernbericht を削除し、ここに FALL-NR. を配置 */}
        ...
        {activeTab === 'analysis' && (
          <div className="space-y-10 animate-in slide-in-from-bottom-2 duration-500 font-sans">
            ...
            <section className="bg-white border border-slate-200 p-8 md:p-12 space-y-12 shadow-sm">
              <div onClick={() => fileInputRef.current?.click()} className="border-2 border-dashed border-slate-100 h-64 flex items-center justify-center cursor-pointer hover:bg-slate-50 transition-all rounded-sm overflow-hidden text-slate-300 text-[9px] font-bold uppercase tracking-widest">
        ...
        {activeTab === 'settings' && (
          <div className="space-y-8 animate-in slide-in-from-bottom-2 duration-500 font-sans">
            <header className="border-b border-slate-100 pb-2 flex justify-between items-end text-[10px] font-bold text-slate-300 uppercase tracking-widest">
               <span>Setup</span>
               <button onClick={() => { saveAllData(); alert("Gespeichert."); }} className="bg-slate-900 text-white px-4 py-2 hover:bg-black">
            ...
            <div className="grid grid-cols-1 gap-6">
              {materials.map((m, idx) => (
                <div key={m.id} className="bg-white border border-slate-200 p-10 flex flex-col gap-8 shadow-sm">
                  ...
                  <div className="grid grid-cols-2 gap-12 text-[9px] font-bold text-slate-300 uppercase">
                    <div><label>Seiten gesamt</label><input type="number" value={m.totalPages} onChange={(e) => updateMaterial(m.id, 'totalPages', Number(e.target.value))} className="w-full text-2xl font-black border-b border-slate-100 outline-none text-slate-900" /></div>
                    <div><label>Tagesziel</label><input type="number" value={m.dailyPace} onChange={(e) => updateMaterial(m.id, 'dailyPace', Number(e.target.value))} className="w-full text-2xl font-black border-b border-slate-100 outline-none text-slate-900" /></div>
                  </div>
                </div>
              ))}
              <button onClick={() => setMaterials([...materials, { id: Date.now().toString(), name: "Material", totalPages: 100, currentPage: 0, dailyPace: 5 }])} className="w-full py-4 border-2 border-dashed border-slate-200 text-slate-300 text-[10px] font-bold uppercase hover:border-slate-950">+ Hinzufügen</button>
            </div>
          </div>
        )}
