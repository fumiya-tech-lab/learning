...
export default function StudyKarteApp() {
  ...
  useEffect(() => {
    setMounted(true);
    const m = localStorage.getItem("karte_final_v12");
    if (m) setMaterials(JSON.parse(m));
    const n = localStorage.getItem("note_final_v12");
    if (n) setStoredReportNote(n);
    const a = localStorage.getItem("ai_final_v12");
  ...
  const saveAllData = (updatedMaterials?: Material[]) => {
    ...
    localStorage.setItem("karte_final_v12", JSON.stringify(dataToSave));
    localStorage.setItem("note_final_v12", storedReportNote);
    localStorage.setItem("ai_final_v12", JSON.stringify(aiExplanations));
  };
  ...
  const runAiAnalysis = async () => {
    ...
    try {
      ...
      const prompt = `
        学習コーチとして、エビングハウスの忘却曲線に基づき、長期記憶を最大化する「復習処方箋」を作成してください。
        本日（${today}）学習した範囲と画像を分析し、以下の形式で3つ出力してください。
        
        【形式】
        ● [復習予定日: 〇月〇日] / [復習範囲: p.〇-〇]
        要点: [日本語で簡潔に100文字以内で解説]

        【指示】
        - 復習日は、今日から1日後、3日後、7日後のいずれかを適切に割り当ててください。
        - 専門的で信頼感のあるトーン（日本語）にしてください。
      `;
      ...
      const points = (await result.response).text().split('\n').filter(l => l.trim().startsWith('●') || l.trim().startsWith('要点'));
      
      setAiExplanations(points);
      setStoredReportNote(inputNote);
      saveAllData(); 
      alert("Analyse abgeschlossen. Der Karte wurde aktualisiert.");
      setActiveTab('karte');
  ...
  return (
    <div className="min-h-screen bg-[#FAFAFA] text-[#1A1A1A] font-serif">
      ...
      {/* 1. Navigation (Bericht -> Karte に変更) */}
      ...
      <main className="max-w-4xl mx-auto p-4 md:p-10 pb-32 print:p-0">
        ...
        {activeTab === 'karte' && (
          <div className="bg-white border border-slate-200 p-10 md:p-16 print:border-none print:p-0 min-h-[290mm] animate-in fade-in duration-700">
            ...
            <footer className="mt-20 pt-4 border-t border-slate-100 text-[8px] font-mono uppercase tracking-widest opacity-20">Studienakte Parallel v3.9</footer>
          </div>
        )}
        ...
        {activeTab === 'analysis' && (
          <div className="space-y-10 animate-in slide-in-from-bottom-2 duration-500">
            <h2 className="text-[10px] font-bold text-slate-300 uppercase tracking-widest border-b pb-2">Abendliche Analyse ＆ Phase A</h2>
            <section className="bg-white border border-slate-200 p-8 md:p-12 space-y-12 shadow-sm">
              <div onClick={() => fileInputRef.current?.click()} className="border-2 border-dashed border-slate-100 h-64 flex items-center justify-center cursor-pointer hover:bg-slate-50 transition-all rounded-sm overflow-hidden">
                {selectedImage ? <img src={selectedImage} alt="Preview" className="h-full object-contain p-2" /> : <p className="text-[9px] text-slate-300 uppercase font-bold text-center">Material-Screenshot <br /> hochladen</p>}
              ...
              <textarea value={inputNote} onChange={(e) => setInputNote(e.target.value)} placeholder="Tagesnotizen (Was haben Sie heute gelernt?)..." className="w-full h-48 p-6 text-sm italic bg-[#FDFDFD] border border-slate-50 outline-none resize-none font-serif" />
              ...
              <button onClick={runAiAnalysis} disabled={isAnalyzing} className="w-full py-5 bg-slate-900 text-white text-[10px] font-bold uppercase tracking-[0.4em] hover:bg-black transition-all">
                {isAnalyzing ? "Analysiere..." : "Sichern ＆ Analyse starten"}
              </button>
            </section>
          </div>
        )}
        ...
        {activeTab === 'settings' && (
          <div className="space-y-8 animate-in slide-in-from-bottom-2 duration-500">
            <header className="border-b border-slate-100 pb-2 flex justify-between items-end">
               <h2 className="text-[10px] font-bold text-slate-300 uppercase tracking-widest">Konfiguration</h2>
               <button onClick={() => { saveAllData(); alert("Daten gespeichert."); }} className="flex items-center gap-2 bg-slate-900 text-white px-4 py-2 text-[10px] font-bold uppercase tracking-widest hover:bg-black transition-all">
            ...
            <div className="grid grid-cols-1 gap-6">
              ...
              <button onClick={() => setMaterials([...materials, { id: Date.now().toString(), name: "Neues Material", totalPages: 100, currentPage: 0, dailyPace: 5 }])} className="w-full py-4 border-2 border-dashed border-slate-200 text-slate-300 text-[10px] font-bold uppercase hover:border-slate-950 transition-all">+ Material hinzufügen</button>
            </div>
          </div>
        )}
